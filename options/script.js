document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function handleFile(file) {
        ...
        try {
            ...
            const parsedContent = await parseResumeWithGemini(base64Data);
    ...
    async function parseResumeWithGemini(base64Data) {
                ...
                (response) => {
                    ...
                    if (response.success) {
                        if (!response.data.content) {
                            reject(new Error('No content generated by AI'));
                            return;
                        }
                        resolve(response.data.content);
                    } else {
                        reject(new Error(response.error || 'Failed to parse resume with AI'));
                    }
                }
            );
        });
